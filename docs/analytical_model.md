...
- **Topology:** Serial-feedback network `BACKLOG → DEV → REVIEW → TESTING → CLOSURE` with a feedback arc from REVIEW and TESTING to DEV for rework, mirroring the Jira transitions (`Reopened`, `Patch Available → In Progress`) documented in the workflow mapping.
- **Nodes:**
  ...
  - **REVIEW queue:** Code-review staging area pulled by **REV**-state developers.
  - **TESTING queue:** Integration/testing queue drained by **TEST**-state developers.
  - **CLOSURE sink:** Absorbing node; no service time.
...
- **Routing:** Tickets normally follow BACKLOG → DEV → REVIEW → TESTING → CLOSURE. REVIEW or TESTING completion may route back to DEV with probability \(P_{\text{REVIEW→DEV}}\) or \(P_{\text{TESTING→DEV}}\) to represent rework noted as "Reopened" in Jira.
- **State alignment:** The developer states DEV, REV, and TEST map one-to-one with the workflow service stages DEV, REVIEW, and TESTING, replacing the legacy combined DEV_REVIEW stage and simplifying queue/state conservation.
...
### 4.3 Routing
1. **Deterministic forward routing:** After service in DEV, tickets proceed to REVIEW with probability 1 unless flagged for abandonment (not modeled here).
2. **Feedback:** REVIEW and TESTING may send tickets back to DEV with routing probabilities \(P_{\text{REVIEW→DEV}}\) and \(P_{\text{TESTING→DEV}}\) that capture "Reopened" Jira events.
3. **Completion:** Tickets exiting TESTING with probability \(1 - P_{\text{TESTING→DEV}}\) move to CLOSURE and leave the system permanently.
...
| \(Q_s\) | Queue length at stage \(s\) | Analytical state vector | Aligns with `BACKLOG, DEV, REVIEW, TESTING`. |
...
2. **Queue conservation:** Let \(\lambda_{DEV}\) be the effective arrival rate to DEV. Then \(\lambda_{DEV} = \lambda_{ext} + \lambda_{REVIEW \to DEV} + \lambda_{TESTING \to DEV}\), where the internal flows satisfy \(\lambda_{REVIEW \to DEV} = P_{\text{REVIEW→DEV}} \cdot \mu_{REV} \cdot c_{REV}\) and similarly for TESTING.
...
5. **Routing probabilities:** Internal routing obeys \(\sum_{k \in \{\text{DEV},\text{REVIEW},\text{TESTING},\text{CLOSURE}\}} P_{s \to k} = 1\) for \(s \in \{\text{DEV},\text{REVIEW},\text{TESTING}\}\), ensuring probability mass conservation.
