...
- $S$ tempo di servizio (o $\mu$ tasso medio di servizio, con $E(S)={1 \over \mu}$)
	- il tempo di servizio può essere modellato secondo diverse distribuzioni (vedi [Equazione KP](#equazione-di-khinchin-pollaczek-(kp)))
- $\lambda$ tasso medio di arrivo
	- gli arrivi sono modellati secondo una distribuzione di Poisson di parametro $\lambda$, dove $\frac{1}{\lambda}$ rappresenta il tempo medio di interarrivo
...
- $T_S$ tempo nel sistema o tempo di residenza/risposta (dall'arrivo nella coda alla terminazione del job)
  **Nota**: nei sistemi a coda singola, i termini "tempo di residenza" e "tempo di risposta" sono sinonimi. Differiscono tuttavia nelle reti di code.
- $N_S$ numero di job nel sistema
- $N_Q$ numero di job nella coda
- $U$ o $\rho$ numero di job in servizio, anche detto **utilizzazione**.

Sulle metriche prima definite, si considerano le seguenti metriche derivate, classificate per tempo e popolazione:
...
Quando si analizza a posteriori una coda di servizio, si possono considerare le **tracce**, ossia le coppie <tempo di arrivo, tempo richiesto di servizio> di ogni job arrivato alla coda. Si parla in questo caso di simulazione guidata da tracce (o *trace-driven simulation*).
...
L'utilizzazione è interpretabile come la probabilità di trovare il servente occupato. Infatti, il servente può servire un solo job alla volta. Un sistema si dice in **equilibrio stazionario** se $\rho<1$.
...
- **scheduling astratto**: dipende da discipline di alto livello e non tiene conto di caratteristiche concrete dei job, ad esempio della dimensione (size). Tra questi rientrano
	...
	- **Priorità (astratta)** -> le richieste sono servite il base alla loro priorità, definita secondo un certo criterio (che non tenga conto della size del job)
- **scheduling size-based**: è una forma di scheduling con priorità che tiene però conto della dimensione dei job (in termini di tempo di servizio) per passare al processamento (ad esempio, Shortest Job First, SJF)
==Alcune misure su questi criteri sono indipendenti dalla dimensione del job.==
...
- **esponenziale**: una sola fase con distribuzione esponenziale di parametro $\mu$
- **k-Erlang**: consiste di k fasi consecutive, ognuna distribuita esponenzialmente con parametro $k\mu$
...
Di seguito sono riportati i modelli delle distribuzioni a fasi con i relativi tassi di servizio (gli inversi dei tempi medi) su ogni fase:
![[Distribuzioni a fasi.png]]
### Leggi note
#### Legge di Little (1961)
Si consideri una coda con disciplina FIFO, capacità infinita e bilanciamento dei flussi (equilibrio stazionario, $\rho<1$). Sia $\lambda$ il tasso di arrivo nel sistema (sconosciuto) e $T$ una finestra temporale di osservazione. Il numero di elementi nel sistema nell'arco temporale $T$ è $N=\lambda T$.
...
$$
dove:
- ${\rho^2 \over (1-\rho)}$ rappresenta il fattore di congestione
	- $\rho^2$ rappresenta la crescita della coda; è al quadrato perché gli effetti del traffico sulla coda sono non lineari, ossia una piccola variazione del traffico quando la coda è non vuota può comportare un incremento sproporzionato della coda
	- $1-\rho$ rappresenta l'avvicinarsi dell'utilizzazione al suo limite (1), ossia l'assenza di "slack time" in cui il server può riprendersi da picchi inaspettati del traffico, smaltendo i job in coda (infatti, per $\rho\to 1$, il termine tende a infinito).
- $\frac{1}{2}$ è un fattore di aggiustamento per la randomicità; infatti, quando una richiesta arriva e trova il servente occupato, non dovrà aspettare l'intero tempo di processamento medio,  bensì - in media - la sua metà.
- $\left[1+\frac{\sigma^2(S)}{E^2(S)}\right]$ è il fattore di variabilità, che rappresenta l'inconsistenza della velocità di servizio
	- $\frac{\sigma^2(S)}{E^2(S)}=C^2$ è il **coefficiente di variabilità**, che aumenta all'aumentare della varianza (variabilità) del sistema
	- $1$ serve affinché il termine non tenda a zero quando il tempo di servizio è poco variabile.
...
$$
**Nota**: nel caso dell'esponenziale, $E(S^2)=2E^2(S)$, dunque $E(T_Q)=\frac{\rho E(S)}{1-\rho}$. Inoltre,
$$
E(T_S)=E(T_Q)+E(S)=\frac{\rho E(S)}{1-\rho}+E(S)=\frac{\rho E(S) + E(S) - \rho E(S)}{1-\rho} = \frac{E(S)}{1-\rho}=\frac{1}{\mu-\lambda}
$$
...
Tutti gli ordini di servizio non-preemptive che non fanno uso delle dimensioni del job hanno la stessa distribuzione sul numero di job nel sistema (che tramite Little si estende anche ai tempi).
$$
E(N_S)
\qquad
E(T_S)
...
$$
**Nota**: $E(T_Q(x))=E(T_Q)~\forall x$ <u>solo</u> per scheduling astratto.
...
$$
In generale, tutti gli scheduling non size-based con prelazione producono lo stesso slowdown medio per i job di qualsiasi dimensione:
$$
...
\rho =
\begin{cases}
{\lambda \over \mu} = \lambda E(S) & \text{se } m=1
\\
{\lambda \over m\mu} = {{\lambda E(S_i)}\over m} & \text{se } m>1
\end{cases}
$$
Si può interpretare $\rho$ nel caso globale (all'equilibrio stazionario) come la percentuale di quegli $m$ serventi che sono occupati. Inoltre, si assume che ogni servente $i$ abbia lo stesso tempo di servizio $E(S_i)={1 \over \mu}$. Tuttavia, questo tempo vale se si considera lo stesso job che entra ed esce dallo stesso servente. Tuttavia, se si vuole conoscere il tempo medio in cui si libera un servente qualunque, ossia il tempo trascorso dall'entrata di un job in un servente all'uscita di un qualunque job da un qualsiasi servente, questo è $E(S)={E(S_i) \over m}={1 \over m\mu}$.

La probabilità di avere $n$ job all'interno del sistema è pari a
$$
p(n) =
\begin{cases}
{1 \over n!} (m\rho)^n p(0) &\text{se } n\leq m
\\
...
- dal punto di vista del tempo di attesa in coda, essendo $\rho\geq P_Q$, il sistema multiserver (Erlang-B) comporta un miglioramento del tempo di attesa rispetto al caso con servente singolo (KP), grazie alla sua capacità distribuita.
- relativamente al tempo di servizio, nel caso multiserver basta considerare quello di un singolo servente, vale a dire $E(S_i)=\frac{1}{\mu}=mE(S)$, mentre nel caso  con servente singolo si ha - ovviamente - solo $E(S)$. Segue che il caso a servente singolo ha un tempo di servizio $m$ volte minore rispetto al caso multiserver.

A causa della differenza tra tempo di attesa in coda e tempo di servizio nei due scenari, si possono fare le seguenti considerazioni relativamente al tempo di risposta:
- per $\rho\to 0$, il multiserver è $m$ volte più lento del single-server
- per $\rho\to 1$, entrambi hanno circa lo stesso tempo di risposta.
##### Fattore di scala
Si consideri un sistema multiserver con $m$ serventi con tasso di arrivo $\alpha\lambda$ e tasso di servizio di ogni servente pari a $\alpha\mu$, $\alpha>1$. Allora, $E(S_i)=\frac{1}{\alpha\mu}$ e 
$$
E(T_Q)_{m,\alpha}=\frac{P_QE(S)_{m,a}}{1-\rho}=\frac{1}{\alpha}\frac{P_QE(S)_{m,1}}{1-\rho}=\frac{1}{\alpha}E(T_Q)_{m,1}
$$
### Scheduling con priorità
Parlando sempre di scheduling astratto, è possibile dividere il flusso entrante in più code. Ogni coda racchiude una **classe di servizio**, ognuna delle quali ha una **priorità** diversa. Questo tipo di scheduling trova applicazione in diverse situazioni:
...
Per ogni classe $k$, sia $S_k$ il tempo di servizio richiesto da un job di tale classe. Si considerano $E(S_k)=E(S)=\frac{1}{\mu}$, $\sigma^2(S_k)=\sigma^2(S)~\forall k$ e $\rho_k=\lambda_k E(S_k)=\lambda_k E(S)$.
...
$$
E(T_{Q_1})^{NP\_priority}=\frac{{\lambda\over 2} E(S^2)}{1-\rho_1}\leq\frac{\frac{\lambda}{2}E(S^2)}{1-\rho}= E(T_Q)^{KP}
$$
	essendo $\rho\geq\rho_1$.
- per la classe con priorità minore:$$
E(T_{Q_r})^{NP\_priority}=\frac{{\lambda\over 2} E(S^2)}{(1-\rho)(1-\sum_{i=1}^{r-1}\rho_i)}\geq\frac{\frac{\lambda}{2}E(S^2)}{1-\rho}= E(T_Q)^{KP}
$$ - prestazioni globali:
$$
...
dato che $\sum_{i=1}^k\lambda_i \leq \lambda$.
...
Nel caso con prelazione, un job può tornare in coda se si presenta un altro job con priorità maggiore. Il ritorno in coda avviene in una classe minore, corrispondente al tempo di servizio rimanente. Tuttavia, il tempo di attesa viene normalmente calcolato in base a quando il job prende servizio per la prima volta. Il restante tempo che si paga è incluso nel **tempo di servizio virtuale**:
$$
...
$$
In generale, non si ha una relazione tra $E(T_S)^{P\_priority}$ e $E(T_S)^{KP}$. Tuttavia, <u>solo</u> nel caso di tempi di servizio esponenziali, vale la relazione
$$
E(T_S)^{P\_priority}=E(T_S)^{KP}
$$
Infatti, considerando per semplicità il caso con due sole classi ($r=2$):
$$
\begin{align}
E(T_S)^{P\_priority}&=p_1E(T_{S_1})+p_2E(T_{S_2})=
\\&=p_1\left[\frac{\frac{\lambda_1}{2}E(S^2)}{1-\rho_1}+E(S)\right]+
p_2\left[\frac{\frac{\lambda}{2}E(S^2)}{(1-\rho)(1-\rho_1)}+ \frac{E(S)}{1-\rho_1}\right]
\\(*)&=p_1\left[\frac{\rho_1E(S)}{1-\rho_1}+E(S)\right]+
p_2\left[\frac{\rho E(S)}{(1-\rho)(1-\rho_1)}+ \frac{E(S)}{1-\rho_1}\right]
\\&=E(S)\left(p_1\left[\frac{\rho_1+1-\rho_1}{1-\rho_1}\right]+
p_2\left[\frac{\rho+1-\rho}{(1-\rho)(1-\rho_1)}\right]\right)
\\&=E(S)\left[\frac{p_1}{1-\rho_1} + \frac{p_2}{(1-\rho)(1-\rho_1)}\right]
\\&=E(S)\frac{p_1(1-\rho)+p_2}{(1-\rho)(1-\rho_1)}
\\&=E(S)\frac{p_1-\rho_1+p_2}{(1-\rho)(1-\rho_1)}
\\&=E(S)\frac{1-\rho_1}{(1-\rho)(1-\rho_1)}=\frac{E(S)}{1-\rho}=E(T_S)^{KP}
\end{align}
$$
Dove il passaggio $(*)$ è stato possibile grazie all'assunzione dei tempi di servizio esponenziali.
##### Multiserver con priorità astratta e prelazione
Si possono unire quanto detto fin'ora sulla priorità astratta e la formula di Erlang-B per ottenere il tempo di attesa in coda in un centro multiserver con scheduling prioritario e prelazione. Ad esempio, nel caso di due classi, da
$$
E(T_Q)^{NP\_priority}=p_1\frac{\rho_1E(S)}{1-\rho_1}+p_2\frac{\rho E(S)}{(1-\rho)(1-\rho_1)}
$$
si deriva
$$
E(T_Q)^{multiserver\_priority}=p_1\frac{P_{Q_1}E(S)}{1-\rho_1}+p_2\frac{P_Q E(S)}{(1-\rho)(1-\rho_1)}
$$
dove $P_{Q_1}$ è la stessa formula di $P_Q$ calcolata con $\rho_1$ al posto di $\rho$. In questo caso, $\rho$ è stata sostituita con $P_Q$ poiché bisogna considerare la probabilità di trovare tutti i serventi occupati. Inoltre, per la classe 1, si considera la probabilità di trovare i serventi occupati da soli job di classe 1, ossia per cui non è possibile effettuare prelazione. D'altra parte, la classe 2 deve considerare i serventi occupati da un job di qualsiasi classe (ossia, $P_Q$), non essendoci altre classi di priorità minore. In generale, la classe $k$ deve considerare la probabilità di trovare tutti i serventi occupati da job di classe $\leq k$, vale a dire calcolare $P_{Q_k}$ sostituendo, nella formula della Erlang-B, $\rho$ con $\sum_{i=1}^k\rho_i$.
#### Priorità size-based
...
$$
\rho_k = \lambda \int_{x_{k-1}}^{x_k}tf(t)dt=\lambda_k E(S_k)
$$
...
$$
E(T_Q)^{SJF}=\frac{\lambda}{2}E(S^2)\int_0^{\infty}\frac{f(x)}{\left(1-\lambda\int_0^x tf(t)dt\right)^2}dx
$$
...
dove
$$
\rho_x=\lambda\int_0^xtf(t)dt=\lambda F(x)\int_0^xt\frac{f(t)}{F(t)}dt
$$
...
In una policy size-based, la prelazione viene applicata solo se il tempo rimanente al job in esecuzione (di classe $h$, cioè $E(S_{h_{rem}})$) è maggiore del tempo richiesto da un qualsiasi job di classe $k<h$. Se la prelazione avviene, il job viene interrotto e posto in una classe con priorità maggiore, considerando solo il tempo rimanente.
...
Si può considerare lo scheduling SJF con prelazione, ottenendo lo scheduling **Shortest Remaining Processing Time** (**SRPT**, anche detto **Shortest Remaining Job First**, **SRJF**). Ciò significa considerare un numero infinito di classi, avendo dunque
$$
...
$$
Condizionatamente alla size, si ha:
$$
...
$$
Per la proprietà di indipendenza, si può calcolare la probabilità di avere $i$ job nel primo centro e $j$ job nel secondo:
$$
...
In questo caso, cade l'ipotesi di indipendenza, non valendo più il teorema di Burke. Si ragiona separatamente per i due sistemi. Se $\rho_i<1$, $i\in\{1,2\}$, si ha sempre che
$$
\begin{align}
&\pi(i,j)=\Pr\{n_1=i\}\Pr\{n_2=j\}, &\forall(i,j)\in E
\\
&\Pr\{n_i=k\}=\rho_i^k(1-\rho_i),  &i\in\{1,2\}
\end{align}
$$
...
\implies
\qquad
\lambda_1=\lambda_2=\frac{\lambda}{p}
$$
Inoltre, il tasso di visita al centro $i$ è dato dal rapporto tra $\lambda_i$ (tasso di arrivo al centro $i$) e il $\lambda$ (tasso di arrivo globale nel sistema), ossia
$$
...
v_1=v_2={1\over p}
$$
Il tempo medio di risposta per il centro $i$ è dato da
$$
R_i=\frac{1}{\mu_i-\lambda_i}
$$
Mentre quello per l'intero sistema è
$$
R=\sum_i v_iR_i
$$
...
- i dispositivi (o centri) devono essere omogenei, ossia il routing deve essere indipendente dal carico delle code, ossia dal tempo di servizio medio dei dispositivi.
...
- $B_i$, il tempo totale per cui il dispositivo $i$ è occupato, ossia in cui $n_i>0$
...
MVA è un metodo per calcolare le misure all'interno di una rete chiusa, alternativo alla forma prodotto. Il procedimento è iterativo e consiste nel formulare il tempo di risposta di ogni centro con $M$ job nel sistema allo stesso valore con $M-1$ job nel sistema. Si ha
$$
E\left[T_j^{(M)}\right]=\frac{1}{\mu_j}+\frac{p_j\lambda^{(M-1)}E\left[T_j^{(M-1)}\right]}{\mu_j}
$$
dove:
- $$p_j=\frac{\lambda_j^{(M-1)}}{\lambda^{(M-1)}}$$	(indipendente da $M$)
- $$\lambda^{(M-1)}=\frac{M-1}{\sum_{j=1}^k p_jE\left[T_j^{(M-1)}\right]}$$
- $\lambda_j^{(M)}$ è il tasso di arrivo dei job al centro $j$ con $M$.

Applicando queste relazioni ricorsivamente a partire da $M=1$, è possibile trovare $E\left[T_j^{(M)}\right]$per qualsiasi $M$. Inoltre, applicando la legge di Little, si può anche trovare il numero medio di job in ogni centro, con
$$
E\left[N_j^{(M)}\right]=E\left[T_j^{(M)}\right]\cdot\lambda_j^{(M)}
$$
con $\sum_{j=1}^k E\left[N_j^{(M)}\right]=M$.
## Dimostrazioni varie
### Priorità astratta senza prelazione
...
### Priorità size-based
#### Prestazioni locali
Si vuole dimostrare che il tempo di attesa in coda dei job in uno scheduling con priorità size-based è minore del caso con lo scheduling con priorità astratta, ossia
$$
E(T_{Q_k})^{SB\_NP}\leq E(T_{Q_k})^{abstract\_NP}
$$
Si ha che
$$
\begin{align}
\left[\left(1-\sum_{i=1}^k\rho_i\right)\left((1-\sum_{i=1}^{k-1}\rho_i\right)\right]^{SB\_NP}
\geq
\left[\left(1-\sum_{i=1}^k\rho_i\right)\left((1-\sum_{i=1}^{k-1}\rho_i\right)\right]^{abstract\_NP}
\end{align}
$$
che è vero essendo
$$
\left[\sum_{i=1}^h\rho_i\right]^{SB\_NP}\leq\left[\sum_{i=1}^h\rho_i\right]^{abstract\_NP}\qquad\forall h
$$
A livello locale, non si può invece stabilire una relazione tra i tempi nel sistema.
#### Prestazioni globali
A livello globale, si può stabilire che
$$
E(T_Q)^{SB\_NP}\leq E(T_Q)^{abstract\_NP}
\implies
E(T_S)^{SB\_NP}\leq E(T_S)^{abstract\_NP}
