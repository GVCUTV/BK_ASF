...
## Prompt 2 — Analyze Lognormal vs Weibull Service-Time Fit Mismatch
...
## Prompt 3 — Fix Missing Column Aliases in 8_export_fit_summary.py

**Goal**  
Fix `etl/8_export_fit_summary.py` so it no longer errors when a stage CSV uses alternate column headers; normalize/alias columns so the script can select a winner and export `fit_summary.csv`.

**Context**  
The script currently requires `Distribuzione`, `Parametri`, and `MAE_KDE_PDF`. The review-stage CSV uses different header names, triggering the missing-columns error. Update the script to normalize column names (case-insensitive, trimmed) and map common variants to the canonical names before validation.

**Files to Read**  
1. `etl/8_export_fit_summary.py` (full file).  
2. If needed, the review-stage CSV header structure for reference (read-only, no changes).

**Objectives**  
1. Add a normalization step after reading each CSV to map alternate column headers to the canonical names `Distribuzione`, `Parametri`, and `MAE_KDE_PDF`.  
2. Support common variants (e.g., `Distribution`, `Distribuzione`, `dist`; `Parametri`, `Params`, `Parameters`; `MAE_KDE_PDF`, `MAE_KDE`, `MAE`).  
3. If required columns are still missing after normalization, log an error that includes available columns and the alias map used, then exit.  
4. Preserve all existing winner-selection logic, parameter parsing, and output schema.

**Output Requirements**  
- Provide a unified diff that edits only `etl/8_export_fit_summary.py`.  
- No new files.  
- The script must continue to write `fit_summary.csv` with the same structure.
...
**Definition of Done**  
- The script handles review-stage CSVs with alternate headers without raising the missing-columns error.  
- Required columns are normalized before downstream logic runs.  
- Error reporting includes available columns and alias map when columns remain missing.  
- Only `etl/8_export_fit_summary.py` is modified.
